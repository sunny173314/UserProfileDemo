            ...
            try
            {
                ...
                if (document != null)
                {
                    userProfile = new UserProfile
                    {
                       ...
                       BirhDate = document.GetString("BirhDate"),
                      Age=document.GetString("Age"),
                        ImageData = document.GetBlob("ImageData")?.Content
                    };
                    //userProfile.Age = document.GetInt("Age");
                }
                // end::docfetch[]
            }
            catch (Exception ex)
        ...
        // end::saveUserProfile[]
        {
            ...
            try
            {
                if (userProfile != null)
                {
                    ...
                    mutableDocument.SetString("FirstName", userProfile.FirstName);
                    mutableDocument.SetString("LastName", userProfile.LastName);
                    mutableDocument.SetString("Email", userProfile.Email);
                  
                    mutableDocument.SetString("Address", userProfile.Address);
                    mutableDocument.SetString("PhoneNumber", userProfile.PhoneNumber);
                    mutableDocument.SetString("Gender", userProfile.Gender);
                    mutableDocument.SetString("BirhDate", userProfile.BirhDate);
                    
                    mutableDocument.SetString("Age", userProfile.Age);
